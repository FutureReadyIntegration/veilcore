...
class CameraMonitor:
    ...
    def heartbeat(self, camera_id: str) -> None:
        ...
        if cam:
            ...
            if cam.state == "offline":
                cam.state = "online"
                self._create_event(camera_id, "feed_restored", "Camera feed restored")
    ...
    def report_feed_lost(self, camera_id: str) -> list[CameraEvent]:
        ...
        return [self._create_event(camera_id, "feed_lost", f"Feed lost from {cam.location}")]
    ...
    def report_tamper(self, camera_id: str, details: str = "") -> list[CameraEvent]:
        ...
        if not cam:
            return []
        cam.state = "tampered"
        return [self._create_event(camera_id, "tamper_detected", details or f"Tamper detected at {cam.location}")]
    ...
    def report_obstruction(self, camera_id: str) -> list[CameraEvent]:
        ...
        return [self._create_event(camera_id, "view_obstructed", f"View obstructed at {cam.location}")]

    def report_ptz_override(self, camera_id: str, details: str = "") -> list[CameraEvent]:
        cam = self._cameras.get(camera_id)
        if not cam:
            return []
        return [self._create_event(camera_id, "ptz_override", details or f"Unauthorized PTZ at {cam.location}")]

    def check_firmware(self, camera_id: str, current_firmware: str) -> list[CameraEvent]:
        cam = self._cameras.get(camera_id)
        if not cam:
            return []
        ...
        if cam.expected_firmware and current_firmware != cam.expected_firmware:
            return [self._create_event(
                ...
                f"Firmware mismatch: expected {cam.expected_firmware}, got {current_firmware}"
            )]
        return []
    ...
    def report_recording_failure(self, camera_id: str) -> list[CameraEvent]:
        ...
        if not cam:
            return []
        cam.is_recording = False
        return [self._create_event(camera_id, "recording_failure", f"Recording failure at {cam.location}")]
    ...
    def _create_event(self, camera_id: str, event_type: str, details: str) -> CameraEvent:
        cam = self._cameras.get(camera_id)
        zone = cam.zone if cam else "general"
        location = cam.location if cam else ""
        severity = EVENT_SEVERITY.get((event_type, zone), DEFAULT_SEVERITY.get(event_type, "medium"))
