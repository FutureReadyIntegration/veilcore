...
class SensorManager:
    ...
    def process_reading(self, sensor_id: str, value: float, unit: str = "") -> list[SensorAlert]:
        ...
        if sensor.threshold_high is not None and value > sensor.threshold_high:
            ...
            alert = SensorAlert(
                ...
                message=f"{sensor.sensor_type} HIGH at {sensor.location}: {value}{unit} > {sensor.threshold_high}{unit}",
        ...
        if sensor.threshold_low is not None and value < sensor.threshold_low:
            ...
            alert = SensorAlert(
                ...
                severity=ZONE_SEVERITY.get(sensor.zone, "medium"),
                alert_type="threshold_breach",
                message=f"{sensor.sensor_type} LOW at {sensor.location}: {value}{unit} < {sensor.threshold_low}{unit}",
    ...
    def process_trigger(self, sensor_id: str) -> list[SensorAlert]:
        ...
        if len(recent) > 5:
            tamper_alert = SensorAlert(
                ...
                message=f"Rapid triggering on {sensor_id} at {sensor.location}: {len(recent)} triggers in 30s",
            )
